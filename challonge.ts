...
const formatRound = (round) => {return round > 0 ? `Winners Round ${round}` : `Losers Round ${round * -1}`};
...
(async function(){
    ...
    const matches = await matchResponse.data;
    ...
    for(var i in matches){
        ...
        let title = `${formatRound(match.round)} ${players[match.player1_id]} vs ${players[match.player2_id]}`;
    ...
    bar.stop();
    console.log("\nFinished: %d Folders created, %d skipped",stats.created,stats.skipped);
    process.exit();
